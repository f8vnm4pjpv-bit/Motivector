...
Motivectorのために具体的に制作されたソースコード、設問文、結果文、画像、キャラクター、デザイン、データその他の表現物には、適用される法令により保護される範囲で、リポジトリ内の独自ライセンスが適用されます。

診断画面や診断結果のスクリーンショット、およびサイトへのリンクは、SNSなどで自由に共有できます。

具体的なソースコード、設問文、画像その他の制作物を実質的に複製して再配布すること、改変版を公開すること、商用利用すること、または別サービスへ流用することは、事前の許可なく行うことはできません。

「Motivector」「ココロの成分表」その他の名称、一般的・学術的な用語、思想、着想、概念、方法または分類そのものについて、独占的な権利を主張するものではありません。本プロジェクトの具体的な表現を複製しない別個の診断、分類またはコンテンツの制作を妨げるものではありません。

法令により認められる引用、私的使用、批評、研究、報道その他の利用は制限されません。第三者のコンテンツや外部ライブラリには、各権利者が定める条件が適用されます。
